...
class TestsPet:
    ...
    def test_post_create_pet_all_fields(self, generate_body_pet, delete_pet_after_test, pet_api):
        pet_body = generate_body_pet(pet_id=True, name=True, photoUrls=True, status=True, tags=True, category=True)
        response = pet_api.post_add_new_pet(body=pet_body)
    ...
    def test_post_create_pet_with_necessary_fields(self, generate_body_pet, delete_pet_after_test, pet_api):
    ...
    def test_post_create_pet_all_status(self, generate_body_pet, delete_pet_after_test, status, pet_api):
        pet_body = generate_body_pet(name=True, photoUrls=True, pet_id=True)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                    "name",
                    dict(photoUrls=True, pet_id=True)
            ),
            (
                    "photoUrls",
                    dict(name=True, pet_id=True)
            ),
        ]
    )
    @allure.title("Cоздание питомца без обязательного поля '{missing_field}'")
    def test_post_create_pet_missing_required_fields(self, generate_body_pet, missing_field, body_kwargs, pet_api):
        pet_body = generate_body_pet(**body_kwargs)
        response = pet_api.post_add_new_pet(body=pet_body)

        with allure.step(f"Статус код ответа с ошибкой 400 или 422"):
    ...
    def test_put_update_pet_all_fields(self, generate_body_pet, create_new_pet, pet_api):
        pet_id = create_new_pet['id']
        pet_body = generate_body_pet(name=True, photoUrls=True, status=True, tags=True, category=True)
        pet_body['id'] = pet_id
        response = pet_api.put_update_pet(body=pet_body)
    ...
    def test_put_update_pet_one_field(self, generate_body_pet, create_new_pet, missing, body_kwargs, pet_api):
        pet_id = create_new_pet['id']
        pet_body = generate_body_pet(**body_kwargs)
        pet_body['id'] = pet_id
        response = pet_api.put_update_pet(body=pet_body)
    ...
    def test_put_update_pet_without_id(self, generate_body_pet, create_new_pet, pet_api):
        pet_body = generate_body_pet(name=True, photoUrls=True, status=True, tags=True, category=True)
        response = pet_api.put_update_pet(body=pet_body)

        with allure.step("Статус код ответа с ошибкой 400 или 404"):
    ...
    def test_get_list_pets_by_status(self, status, pet_api):
        response = pet_api.get_list_pets_by_status(status=status)
    ...
    def test_get_list_pets_in_minor_status(self, pet_api):
        response = pet_api.get_list_pets_by_status(status="status")
    ...
    def test_get_pet_by_id(self, create_new_pet, pet_api):
        pet_body = create_new_pet
        response = pet_api.get_pet_by_id(pet_id=pet_body["id"])
    ...
    def test_get_pet_by_invalid_id(self, pet_api):
        response = pet_api.get_pet_by_id(pet_id=None)

        with allure.step("Статус код с ошибкой 400 или 404"):
            assert response.status_code in (400, 404), f"Не верный статус ответа: {response.text}"

    @allure.title("Обновить имя и статус питомца через форму")
    def test_post_update_pet_name_and_status_with_form(self, create_new_pet, pet_api):
        pet_body = create_new_pet
        name = generate_randon_name()
        status = generate_randon_status_pet()
        response = pet_api.post_update_pet_with_form(pet_id=pet_body["id"], name=name, status=status)
        ...
        with allure.step("Ответ соответствует модели ApiResponse"):
            ...
            assert response.message == str(pet_body["id"])

        get_pet_response = pet_api.get_pet_by_id(pet_id=pet_body["id"]).json()

        with allure.step("Проверка совпадения значения name, с указанным значением в форме"):
            assert get_pet_response["name"] == name

        with allure.step("Проверка совпадения значения status, с указанным значением в форме"):
            assert get_pet_response["status"] == status

    @allure.title("Обновить только имя питомца через форму")
    def test_post_update_pet_name_with_form(self, create_new_pet, pet_api):
        pet_body = create_new_pet
        name = generate_randon_name()
        response = pet_api.post_update_pet_with_form(pet_id=pet_body["id"], name=name)
    ...
    @allure.title("Обновить имя и  статус питомца через форму, указав несуществущий статус")
    def test_post_update_pet_invalid_status_with_form(self, create_new_pet, pet_api):
        pet_body = create_new_pet
        name = generate_randon_name()
        status = generate_randon_name()
        response = pet_api.post_update_pet_with_form(pet_id=pet_body["id"], name=name, status=status)

        with allure.step("Стату код с ошибкой 400"):
            assert response.status_code == 400, f"Не верный статус ответа: {response.text}"

    @allure.title("Удалить питомца")
    def test_delete_pet_by_id(self, create_new_pet, pet_api):
        pet_body = create_new_pet
        response = pet_api.delete_pet(pet_id=pet_body["id"])
        ...
        with allure.step("Удаленный питомец не отдается в списке по id"):
            get_response = pet_api.get_pet_by_id(pet_id=pet_body["id"])
    ...
    def test_delete_pet_by_id_does_not_exist(self, create_new_pet, pet_api):
    ...
    def test_upload_pet_image(self, create_new_pet, temp_image_file, pet_api):
        pet_body = create_new_pet
        file_path = temp_image_file
        response = pet_api.post_upload_pet_image(pet_id=pet_body["id"], file_path=file_path,
        ...
        with allure.step("Проверка, что файл действительно добавился"):
            assert len(get_pet_response["photoUrls"]) == len(pet_body["photoUrls"]) + 1, ('Не добавилась новая ссылка '
                                                                                          'на файл в список photoUrls')
