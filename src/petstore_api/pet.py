...
class PetAPI:
    ...
    def post_update_pet_with_form(self, pet_id: int | None, **kwargs) -> Response:
        ...
        header = dict(self.header, **{'Content-Type': 'application/x-www-form-urlencoded'})
        return self._client.post(path=url_path, params=kwargs, header=header)
    ...
    def post_upload_pet_image(self, pet_id: int, file_path: str, **kwargs):
        ...
        with open(file_path, "rb") as file:
            file_data = {'file': file}
            return self._client.post(path=url_path, header=self.header, file=file_data, params=kwargs)